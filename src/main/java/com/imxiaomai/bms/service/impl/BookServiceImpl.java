...
//                rebook.setBrlist(brm);
                log.info("*******************开始查询此书已借人数，图书ID：" + rebook.getId() + "***********************************");
                ...
                if (rebb != null) {
                    ...
                    log.info("*******************获取正在预约的用户，图书ID：" + rebook.getId() + "***********************************");
    ...
    public Book saveOrUpdateInfo(Book book) throws Exception {
        log.info("添加或修改图书,参数 " + JSON.toJSONString(book));
    ...
    public Book selectSimpleInfoByExample(Book book) throws Exception {
        log.info("查询书籍基本信息,参数 " + JSON.toJSONString(book));
        if (!ObjectUtils.isEmpty(book)) {
            book.setYn(1);
    ...
    public ResMsg<String> discuss(BookDiscuss bookDiscuss) throws Exception {
        log.info("*******************用户开始评论图书，图书ID" + bookDiscuss.getBookId() + "*************************");
        ResMsg<String> msg = new ResMsg<String>();
        bookDiscuss.setYn(1);
        ...
        if (add > 0) {
            ...
            msg.setCode(DATA_NOT_FOUND);
            msg.setDes("评论失败");
            log.info("*******************评论失败，图示ID：" + bookDiscuss.getBookId() + "*************************");
        }
        return msg;
    }
